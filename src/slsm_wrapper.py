...
@dataclass
class FlatState: # this type of memory state is to mplement a flat (against 'structured') memory
    """
    Non-structured semantic memory:
    a single evolving textual note + lightweight plan signal.
    """
    memory_text: str = ""
    ...
    def to_flat_note(self, max_chars: int = 2000) -> str:
        txt = (self.memory_text or "").strip()
        if not txt:
            return ""
...
class SLSMConfig:
    ...
    memory_mode: Literal["structured", "flat"] = "structured"
    ...
    # Flat memory parameters
    flat_note_max_chars: int = 2000
    flat_max_bullets: int = 12
...
class SLSMWrapper:
    ...
    def generate_last_turn(
        ...
        **gen_kwargs,
        ...
        Supports both structured SLSM state and flat summary state under identical injection policies.
        """
        ...
        is_flat = (getattr(self.cfg, "memory_mode", "structured") == "flat")

        # --- Track state (flat or structured) ---
        if is_flat:
            flat_controller = FlatMemController(self.controller.llm, self.cfg)
            state = self._track_flat_state(original_conversation, flat_controller)
        else:
            state = self.track_state(original_conversation)

        # =========================
        # (A) inject != on_detection
        # =========================
        if self.cfg.inject != "on_detection":
            if is_flat:
                msgs = self.build_final_messages_flat(
        ...
        # =========================
        # (B) on_detection: 2-pass
        # =========================
    
        # pass 0: no injection
        if is_flat:
            msgs0 = self.build_final_messages_flat(
                original_conversation,
                state,
                system_prompt=system_prompt,
        ...
        # detection: feed y0 back to controller, decide mismatch
        if is_flat:
            # reuse the SAME flat_controller model/cfg; create a fresh controller object here to avoid
            # relying on outer scope if you refactor
            flat_controller = FlatMemController(self.controller.llm, self.cfg)
            st_after = self._detect_mismatch_by_flat_state_update(
                prev_state=state,
        ...
        # mismatch => inject using UPDATED state and regenerate once
        if is_flat:
            msgs1 = self.build_final_messages_flat(
                original_conversation,
                st_after,
                system_prompt=system_prompt,
        ...
        # if self.cfg.memory_mode == "flat":
        #     controller = FlatMemController(self.controller.llm, self.cfg)
        #     flat_state = self._track_flat_state(original_conversation, flat_controller)
        #     state = flat_state
        # else:
        #     state = self.track_state(original_conversation)
...
#         return bool(obj.get("mismatch", False))
    ...
    def _track_flat_state(
        self,
        conversation: List[Dict[str, str]],
        controller: "FlatMemController",
    ) -> "FlatState":
        st: Optional[FlatState] = None
        history: List[Dict[str, str]] = []
        for msg in conversation:
            st = controller.update(history=history, new_msg=msg, prev=st)
            history.append(msg)
        return st or FlatState()

    def build_final_messages_flat(
        self,
        original_conversation: List[Dict[str, str]],
        state: "FlatState",
        system_prompt: Optional[str] = None,
        ...
        if inject_override is not None:
            ...
            if self.cfg.inject == "always":
                ...
                inject_note = False
            elif self.cfg.inject == "on_risk" and mode in getattr(self.cfg, "risk_modes", ("verify", "clarify")):
                inject_note = True
    
        if inject_note:
            # budget: use a config similar to note_max_items in structured; here we use max_chars
            max_chars = getattr(self.cfg, "flat_note_max_chars", 2000)
            note = state.to_flat_note(max_chars=max_chars) if hasattr(state, "to_flat_note") else (state.memory_text or "")
            note = (note or "").strip()
            if note:
                msgs.append({
                    "role": "system",
                    "content": f"[FLAT MEMORY NOTE]\n{note}\n\nFollow the note above."
                })
    
        msgs.extend(original_conversation)
        return msgs

    def _detect_mismatch_by_flat_state_update(
        self,
        prev_state: "FlatState",
        original_conversation: List[Dict[str, str]],
        y0: str,
        controller: "FlatMemController",
    ) -> Optional["FlatState"]:
        """
        Feed y0 back to flat controller as a NEW assistant message.
        mismatch iff updated plan.mode is verify/clarify.
        """
        try:
            st_after = controller.update(
                history=original_conversation,
                new_msg={"role": "assistant", "content": y0},
                prev=prev_state,
            )
        except Exception:
            # fail-safe: treat as mismatch => force inject+regenerate once
            return prev_state
    
        # (1) explicit mismatch signal takes priority
        if getattr(st_after, "mismatch", False):
            return st_after
        
        # (2) fallback: use plan.mode (backward compatible)
        mode = (st_after.plan or {}).get("mode", "proceed")
        return st_after if mode in ("verify", "clarify") else None
